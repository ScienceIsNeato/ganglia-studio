...
def read_file_contents(file_path: str, encoding: str = "utf-8") -> str | None:
...
def _upload_final_video(local_file_path: str) -> bool:
    ...
    if success:
        ...
        Logger.print_info(
            f"Successfully uploaded final video to GCS: gs://{bucket_name}/{gcs_path}"
        )
...
def concatenate_video_segments(
    video_segments: list[str], output_dir: str, force_reencode: bool = False
) -> str | None:
    ...
    try:
        ...
        base_cmd = ["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", concat_list_path]
...
def add_background_music(
    video_path: str, music_path: str, output_dir: str, music_volume: float = 0.3
) -> str | None:
...
def assemble_final_video(
    ...
    closing_credits_lyrics: str | None = None,
) -> str | None:
...
def generate_closing_credits(
    ...
    try:
        ...
        cmd = (
            ...
            + SLIDESHOW_VIDEO_ARGS
            + AUDIO_ENCODING_ARGS
            + ["-t", str(duration), initial_credits_video_path]
        )
...
def create_video_with_captions(
    segments: list[dict[str, str]], output_path: str, thread_id: str | None = None
) -> str | None:
        ...
        for i, segment in enumerate(segments):
            try:
                ...
                captions = create_word_level_captions(
                    segment["audio"], segment["text"], thread_id=thread_id
                )
                if not captions:
                ...
                if not video_path:
                    raise ValueError(f"Failed to create video for segment {i + 1}")
