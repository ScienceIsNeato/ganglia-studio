...
class FoxAISunoBackend(MusicBackend, SunoInterface):
    ...
    def __init__(self):
        ...
        if not self.api_key:
            raise OSError("Environment variable 'FOXAI_SUNO_API_KEY' is not set.")
    ...
    def check_progress(self, job_id: str) -> tuple[str, float]:
        ...
        try:
            ...
            validation_error = self._validate_query_response(response_data, job_id)
            if validation_error:
            ...
            file_type = self._determine_file_type(meta_data.get("prompt", ""))

            return self._interpret_progress_status(status, meta_data, file_type, job_id)

        except Exception as e:
            return f"Error: {str(e)}", 0

    def _validate_query_response(self, response_data, job_id):
        """Validate query response format and data presence."""
        if not isinstance(response_data, list):
            return "Error: Invalid response format"

        song_data = next((item for item in response_data if item.get("id") == job_id), None)
        if not song_data:
            return "Error: Song data not found"

        return None

    def _determine_file_type(self, prompt):
        """Determine output file type based on prompt content."""
        is_closing_credits = "with lyrics" in prompt.lower() if prompt else False
        return "closing_credits.mp3" if is_closing_credits else "background_music.mp3"

    def _interpret_progress_status(self, status, meta_data, file_type, job_id):
        """Interpret status and return progress tuple."""
        if status == "complete":
            return "Complete", 100
        ...
        estimated_progress = min(95, (elapsed / 180) * 100)
    ...
    def _start_lyrical_song_job(
        ...
        *,
        ...
        try:
            lyrics_data = self._get_lyrics_data(story_text, query_dispatcher)
            style, lyrics = self._extract_style_and_lyrics(lyrics_data)
            actual_duration = duration if duration is not None else self.DEFAULT_CREDITS_DURATION
            model = self._validate_foxai_model(model)
            endpoint = f"{self.api_base_url}/gateway/generate"
            ...
            return self._extract_song_id_from_response(response)

        except Exception as e:
            Logger.print_error(f"Failed to start lyrical song job: {str(e)}")
            return None

    def _get_lyrics_data(self, story_text, query_dispatcher):
        """Get lyrics data from query dispatcher or use story text directly."""
        return query_dispatcher.send_query(story_text) if query_dispatcher else story_text

    def _extract_style_and_lyrics(self, lyrics_data):
        """Extract style and lyrics from lyrics data."""
    ...
    def _validate_foxai_model(self, model):
        """Validate and normalize FoxAI model name."""
        if not model or model.lower() not in ["chirp-v3-5", "chirp-v4"]:
            Logger.print_warning(f"Invalid model '{model}', defaulting to 'chirp-v3-5'")
            return "chirp-v3-5"
        return model

    def _build_lyrical_prompt(self, duration, style, title, lyrics):
        """Build enhanced prompt for lyrical generation."""
        enhanced_prompt = f"Create a {duration}-second {style} song"
        if title:
            enhanced_prompt = f"{enhanced_prompt} titled '{title}'"
    ...
    def _log_request_details(self, endpoint, data, lyrics_data):
    ...
    def _extract_song_id_from_response(self, response):
        """Extract song ID from successful API response."""
        response_data = response.json()
        if response_data.get("code") != 0:
            return None
